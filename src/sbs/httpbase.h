...
// Attach HttpBase to a rtc::StreamInterface which represents a bidirectional HTTP
...
public:
  ...
  bool attach(rtc::StreamInterface* stream);
  rtc::StreamInterface* stream() { return http_stream_; }
  rtc::StreamInterface* detach();
...
//  rtc::StreamInterface* GetDocumentStream();
...
protected:
  ...
  void OnHttpStreamEvent(rtc::StreamInterface* stream, int events, int error);
  void OnDocumentEvent(rtc::StreamInterface* stream, int events, int error);
...
private:
  ...
  HttpData* data_;
  IHttpNotify* notify_;
  rtc::StreamInterface* http_stream_;
