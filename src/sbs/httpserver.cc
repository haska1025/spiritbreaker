...
HttpServer::~HttpServer() {
  ...
  for (ConnectionMap::iterator it = connections_.begin();
       it != connections_.end();
       ++it) {
    rtc::StreamInterface* stream = it->second->EndProcess();
    delete stream;
    delete it->second;
  }
}
...
int
HttpServer::HandleConnection(rtc::StreamInterface* stream) {
...
HttpServer::Connection::~Connection() {
  ...
  rtc::Thread::Current()->Dispose(current_);
}

void
HttpServer::Connection::BeginProcess(rtc::StreamInterface* stream) {
  base_.notify(this);
  base_.attach(stream);
...
HttpServer::Connection::onHttpComplete(HttpMode mode, HttpError err) {
  ...
  if (err != HE_NONE) {
    ...
    rtc::Thread::Current()->Dispose(current_->response.document.release());
...
int HttpListenServer::Listen(const rtc::SocketAddress& address) {
...
bool HttpListenServer::GetAddress(rtc::SocketAddress* address) const {
  if (!listener_) {
    return false;
  }
...
void HttpListenServer::OnReadEvent(rtc::AsyncSocket* socket) {
  RTC_DCHECK(socket == listener_.get());
  rtc::AsyncSocket* incoming = listener_->Accept(nullptr);
  if (incoming) {
    rtc::StreamInterface* stream = new SocketStream(incoming);
