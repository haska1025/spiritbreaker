...
public:
  ...
  int HandleConnection(rtc::StreamInterface* stream);
  // Due to sigslot issues, we can't destroy some streams at an arbitrary time.
  sigslot::signal3<HttpServer*, int, rtc::StreamInterface*> SignalConnectionClosed;
...
public:
  ...
  int Listen(const rtc::SocketAddress& address);
